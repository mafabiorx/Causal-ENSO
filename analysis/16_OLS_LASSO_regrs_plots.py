"""
Plot multivariate regression results produced by script 17 (OLS/Fixed-LASSO version).
...
# ----
# Point to the directory where script 17 saved its results
...
# Season codes as produced by script 17
...
def load_regression_results(pathway: str, target_var: str, lag_range_str: str, method: str
    ...
    Constructs the filename exactly as saved by script 17.
    ...
    # Construct the filename exactly as saved by script 17
...
def plot_predictor_effect(
    ...
    secondary_sign_level: Optional[float] = None,  # Secondary significance level (e.g., 90)
    secondary_alpha: float = 0.4                   # Transparency for secondary layer
) -> None:
    """
    ...
    if single_panel_mode:
        ...
        for season_code in seasons_to_plot:
            ...
            try:
                ...
                # --- Secondary significance layer (if enabled, OLS only) ---
                if method.upper() == 'OLS' and secondary_sign_level is not None and pvals is not None:
                    p_value_secondary = pvals.sel(season=season_code, predictor=predictor_name)
                    regr_var_secondary = regr_coefs.sel(season=season_code, predictor=predictor_name)
                    regr_var_secondary = regr_var_secondary.where(p_value_secondary < (1 - secondary_sign_level / 100.0))
                    regr_var_secondary_scaled = regr_var_secondary / settings['conv_factor']

                    if not regr_var_secondary_scaled.isnull().all():
                        regr_var_secondary_cyclic = add_cyclic_point_xr(regr_var_secondary_scaled)

                        contourf_kwargs_secondary = {
                        ...
                        if use_rasterized:
                            contourf_kwargs_secondary['rasterized'] = True

                        ax.contourf(regr_var_secondary_cyclic.longitude, regr_var_secondary_cyclic.latitude,
                                    regr_var_secondary_cyclic.values, **contourf_kwargs_secondary)

                # --- Primary significance layer ---
                contourf_kwargs = {
                    'transform': ccrs.PlateCarree(),
                    'levels': settings['levels'],
                    'cmap': settings['cmap'],
    ...
    # Dynamic colorbar sizing based on number of panels
    if n_seasons <= 1:
        cbar_fraction = 0.08
        bottom_adjust = 0.23
    elif n_seasons <= 2:
    ...
    if cf_mappable is not None:
        cbar = fig.colorbar(cf_mappable, ax=axs, orientation='horizontal', pad=0.10, fraction=cbar_fraction)
        cbar.set_label(settings['label'], fontsize=16)
        cbar.ax.tick_params(labelsize=14)
    else:
    ...
    plt.subplots_adjust(left=0.04, bottom=bottom_adjust, right=0.96, top=0.95, hspace=0.16)
...
def get_wind_vectors(wind_data: Dict[str, xr.DataArray],
    ...
    Applies scaling for WAF vectors similar to script 9.
    """
    ...
    # --- START: WAF Scaling Logic (adapted from script 9) ---
    if wind_level == 'WAF':
        ...
        # Define scaling parameters (same as script 9)
        min_len = 0.6
        max_len = 1.4
...
def process_variable(var_name: str,
                     ...
                     panel_suffix: str = '',
                     secondary_sign_level: Optional[float] = None,
                     secondary_alpha: float = 0.4) -> None:
    """
    ...
    for predictor in predictors_to_plot:
        plot_predictor_effect(
            ...
            secondary_sign_level=secondary_sign_level,
...
def main(args=None):
    ...
    parser.add_argument('--secondary_sign_level', type=float, default=None,
                        help='Secondary significance level (e.g., 90) for additional context. '
                             'Regions significant at this level but not at primary level shown with transparency.')
    parser.add_argument('--secondary_alpha', type=float, default=0.4,
                        help='Transparency for secondary significance regions (default: 0.4)')
                ...
                for wind_level in effective_wind_levels:
                    for upper_wind_level in upper_wind_levels:
                        ...
                        process_variable(
                            ...
                            secondary_sign_level=parsed_args.secondary_sign_level,
                            secondary_alpha=parsed_args.secondary_alpha
                        )
                        gc.collect()
