"""
Optimization Workflow for PCMCIplus Analysis using MCA

This script implements an optimization workflow to find the optimal MCA domains
that maximize the causal effect of specific MCA modes on specific target ENSO diversity variables at specific lags.

The workflow iteratively:
1. Modifies the vertices in script 2_MCA_prec_RWS_200_*.py
2. Runs the MCA analysis via script 2
3. Preprocesses the data via script 14
4. Runs the PCMCIplus analysis via script 16
5. Extracts the causal effect values and updates the vertices for the next iteration
"""
...
SUFFIX = 'all_lags_iterative_step1_noFDR_MCA_prec_RWS_200'
VERSION_BASE = 'v'
START_VERSION_NUM = 0
TRIAL = 407
RESULTS_DIR = get_results_path(f'MCA_prec_RWS_200/trial_{TRIAL}/')
os.makedirs(RESULTS_DIR, exist_ok=True)

INITIAL_RECT = Rect(lat_min=-30, lat_max=-15, lon_min=-55, lon_max=-35)
...
# Define the list of specific MCA variables we are optimizing
OPTIMIZED_MCA_VARS = ['MCA1_prec', 'MCA2_prec', 'MCA1_RWS_200', 'MCA2_RWS_200']
...
def prepare_target_directories(base_results_dir: str, target_descriptor: str, version: str) -> str:
    ...
    os.makedirs(os.path.join(target_version_dir, 'NetCDFs_MCA_modes'), exist_ok=True)
...
def run_optimization_step(individual_vertices: Sequence[Tuple[int, int]], 
    ...
    script2_orig_path = os.path.join(CURRENT_DIR, '2_MCA_prec_RWS_200.py')
    script14_orig_path = os.path.join(CURRENT_DIR, '14_save_ds_caus_E_v_C_40_24_MCA_prec_RWS_200.py')
    script16_orig_path = os.path.join(CURRENT_DIR, '16_PCMCIplus_all_lags_40_24_iterative_step1_noFDR_MCA_prec_RWS_200.py')
...
def genetic_algorithm_optimization():
    print("Starting MCA genetic algorithm optimization...")
    ...
    for target_key, targets_config in ALL_TARGETS.items():
        ...
        print(f"Optimizing for variables: {OPTIMIZED_MCA_VARS}")
        ...
        target_tracking = {
            ...
            'variable_best_effects': {var_name: 0.0 for var_name in OPTIMIZED_MCA_VARS}
        }
        ...
        for generation in range(GENERATIONS):
            ...
            with concurrent.futures.ProcessPoolExecutor(
                ...
                for fut in concurrent.futures.as_completed(futs):
                    ...
                    for var_name_log_entry in OPTIMIZED_MCA_VARS:
                    ...
                    for var_name_track in OPTIMIZED_MCA_VARS:
        ...
        if target_tracking['best_vertices']:
            ...
            for var_name_l in OPTIMIZED_MCA_VARS:
    ...
    if not all_targets_tracking:
        ...
        for target_key_s, target_data_s in all_targets_tracking.items():
            ...
            for var_name_s_detail in OPTIMIZED_MCA_VARS:
...
if __name__ == "__main__":
    print(f"Starting MCA optimization workflow for Trial {TRIAL}...")
    ...
    print(f"Optimizing variables: {OPTIMIZED_MCA_VARS}")
