...
# Analysis parameters
MAX_COMBINATIONS = 10 # Max number of conditioning variables in PCMCI+
TAU_MIN = 1           # Minimum time lag to consider
TAU_MAX = 6           # Maximum time lag to consider

# Define paths using target descriptor
VERSION = os.environ.get("VERSION", 'v0')
SUFFIX = 'all_lags_iterative_step1_noFDR_REOF_SST' # Suffix for output files

target_dir_results_base = get_results_path(f'REOF_SST/{trial}/{target_descriptor}/{VERSION}')
...
reof_season, reof_group_name = "SON", "SON_PREDICTORS" 
if target_lag is not None:
    reof_season, reof_group_name = get_season_from_lag(target_lag)
    logging.info(f"Using {reof_season} season for REOF variables (assigned to group {reof_group_name}) based on target_lag: {target_lag}")
else:
    logging.warning("No target_lag found or parsed. Using SON as default season for REOF variables.")

VARIABLE_SEASONS = {
    'DMI_JJA': 'JJA', 'SIOD_SON': 'SON',
...
for i in range(1, 5): 
    VARIABLE_SEASONS[f'PC{i}_SST'] = reof_season

ALL_VARIABLES = list(VARIABLE_SEASONS.keys()) 

JJA_PREDICTORS = ['DMI_SON']
SON_PREDICTORS = ['SIOD_SON', 'MCA2 prec-RWS SON']
...
reof_vars_list = [f'PC{i}_SST' for i in range(1, 5)]
if reof_group_name == "JJA_PREDICTORS": JJA_PREDICTORS.extend(reof_vars_list)
elif reof_group_name == "SON_PREDICTORS": SON_PREDICTORS.extend(reof_vars_list)
elif reof_group_name == "DJF_CONFOUNDERS": DJF_CONFOUNDERS.extend(reof_vars_list)
elif reof_group_name == "MAM_MEDIATORS": MAM_MEDIATORS.extend(reof_vars_list)
else: 
    logging.warning(f"REOF group name '{reof_group_name}' not recognized. REOF vars may not be in a defined group list.")

SEASON_MONTHS = {'JJA': [7], 'SON': [10], 'DJF': [1], 'MAM': [4], 'DJF_effect': [1]}
...
def track_edge_significance(results_list: List[Dict], var_names: List[str],
    ...
    """
    edge_records = []
    for run_idx, (run_results, alpha) in enumerate(zip(results_list, alpha_values)):
        val_matrix = run_results['val_matrix']
        p_matrix = run_results['p_matrix']
        graph = run_results['graph'] 

        for i in range(len(var_names)):      
            for j in range(len(var_names)):  
                for tau_idx in range(TAU_MIN, TAU_MAX + 1): 
                    if graph[i, j, tau_idx] != '': 
                        edge_records.append({
                            'cause': var_names[i],
                            'effect': var_names[j],
                            'lag': tau_idx, 
                            'alpha': alpha, 
                            'causal_effect': val_matrix[i, j, tau_idx],
                            'p_value': p_matrix[i, j, tau_idx]
                        })
...
def main() -> None:
    ...
    data_file_path = os.path.join(target_dir_results_base, f'ds_caus_REOF_40_24_gen{generation}_ind{individual}.nc')
    ...
    # Single-test persistence analysis
